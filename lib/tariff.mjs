...
export function buildMessages({ result, bill, month, tariff, sourceUri, cheeky = false }) {
  ...
  const openEn = cheeky ? "Apparently my electricity comes with a VIP upgrade 😤" : `Hi, I checked the ${monthEn} bill against the Ministry's published tariff.`;
  const openAr = cheeky ? "شكلها الكهربا عندي صارت VIP 😤" : `مرحبا، راجعت فاتورة ${monthAr} مع تسعيرة الوزارة.`;
  return {
    en: cheeky
      ? `${openEn}\nMinistry says ${formatLL(rate)} LL/kWh and ${formatLL(fixed)} LL fixed for ${bill.amps} amps. My bill says ${formatLL(lines.rate.charged)} LL and ${formatLL(lines.fixed.charged)} LL. That's ${formatLL(result.overchargeLL)} LL${usd} extra on ${formatLL(bill.kwh)} kWh. Can we fix the VIP part?${src}`
      : `${openEn}\nIt's ${formatLL(rate)} LL per kWh and ${formatLL(fixed)} LL fixed for ${bill.amps} amps, but my bill has ${formatLL(lines.rate.charged)} LL and ${formatLL(lines.fixed.charged)} LL. On ${formatLL(bill.kwh)} kWh that's ${formatLL(result.overchargeLL)} LL${usd} extra. Can you take a look and adjust it?${src}`,
    ar: cheeky
      ? `${openAr}\nالوزارة حاطة ${formatLL(rate)} ل.ل. للكيلوواط و${formatLL(fixed)} ل.ل. مقطوعية لـ${bill.amps} أمبير. فاتورتي ${formatLL(lines.rate.charged)} ل.ل. و${formatLL(lines.fixed.charged)} ل.ل. يعني ${formatLL(result.overchargeLL)} ل.ل.${usdAr} زيادة على ${formatLL(bill.kwh)} كيلوواط. منصلّح قصة الـVIP؟${srcAr}`
      : `${openAr}\nالتسعيرة ${formatLL(rate)} ل.ل. للكيلوواط و${formatLL(fixed)} ل.ل. مقطوعية لـ${bill.amps} أمبير، بس فاتورتي ${formatLL(lines.rate.charged)} ل.ل. و${formatLL(lines.fixed.charged)} ل.ل. على ${formatLL(bill.kwh)} كيلوواط في ${formatLL(result.overchargeLL)} ل.ل.${usdAr} زيادة. فيك تشوفها وتصحّحها؟${srcAr}`,
  };
}
