...
export function buildReport(input) {
  ...
  const includePrintedTotals = input.printedTotalsConfirmed === true;
  if (includePrintedTotals && (!Number.isFinite(bill.totalLL) || !Number.isFinite(bill.fixedLL) || bill.totalLL < bill.fixedLL)) {
    throw new ContributeError("Confirm the printed total and standing charge. Missing charges are unknown, not zero.");
  }
  ...
  notes.push(includePrintedTotals
    ? "User confirmed the printed total is before VAT and contains only energy plus the standing charge. Read from a photographed bill by Moteur Check."
    : "Read from a photographed bill by Moteur Check. Printed total not confirmed for price reconciliation; VAT unknown.");
  const body = {
  ...
  // Both are independently read figures, never kWh * rate. Send neither if uncertain:
  // the partner treats an omitted standing_charge as zero when printed_total is present.
  if (includePrintedTotals) {
    body.printed_total = bill.totalLL;
    body.standing_charge = bill.fixedLL;
  }
  // Do not manufacture energy_total = kWh * price: it cannot independently verify OCR.
  // Bill photo, personal details, VAT status and our verdict are not submitted.
  return body;
}
